			 ...
			 <div class="row">
                    <div class="col-md-12">
                        <h1 class="page-header">
                           Update Medicine Stock<small> </small>
                        </h1>
                    </div>
                </div> 
            <div class="row">
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                                    ...
                                    if (isset($_GET['available'])) 
                                    {
                                        ...
                                        if ($con->query("update ordermed set stock='Available' where id = '$_GET[available]'"))
                                        {
                                        header("location:adminmstock.php");
                                        }
                                    }  
                                    if (isset($_GET['unavailable'])) 
                                    {
                                        $con = mysqli_connect('localhost','root','','petcare');
                                        if ($con->query("update ordermed set stock='Unavailable' where id = '$_GET[unavailable]'"))
                                        {
                                        header("location:adminmstock.php");
                                        }
                                    } 
									...
									<?php
										...
										while($row = mysqli_fetch_array($re))
										{
                                            ...
                                            <td>".$row['pet']."</td>
                                            <td>".$row['stock']."</td>
                                            <td> <a href=adminmstock.php?available=".$row['id']."<button class='btn btn-primary'> <i class='fa fa-edit' ></i>Available</button> </td>
                                            <td> <a href=adminmstock.php?unavailable=".$row['id']."<button class='btn btn-danger'> <i class='fa fa-edit' ></i>Unavailable</button> </td>
